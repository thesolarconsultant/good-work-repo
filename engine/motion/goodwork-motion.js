/* ============================================================================
   ...
   Cheat sheet (add to any element):
     ...
     data-om-stagger           on a parent: stagger its [data-om-reveal] kids
     data-om-count="80"        count up to a number when in view (suffix kept)
     data-om-ticker="1284"     odometer roll
     data-om-typing="Hello."   typewriter (add class om-caret for the cursor)
     data-om-rotate="a,b,c"    cycle words in place
     data-om-scramble="TEXT"   decode-from-noise
     data-om-magnetic          lean toward the cursor
     data-om-particles         fill an element/canvas with drifting motes
     data-om-meteors="9"       spawn N falling streaks (element needs .om-meteors)
     class="om-ripple"         ripple from the click point
   Plus pure-CSS classes (no JS): om-shimmer om-beam om-pulse om-glow om-rainbow
     om-aurora om-gradient-text om-shiny-text om-marquee om-float om-orb
     om-border-beam om-shine-border om-dots
   ========================================================================== */
(function (global) {
  "use strict";
  ...
  var M = {
    reveal: function (root) {
      each(root, "[data-om-stagger]", function (p) {
        Array.prototype.forEach.call(p.querySelectorAll("[data-om-reveal]"), function (k, i) {
          k.style.transitionDelay = (i * 0.09) + "s";
        });
      });
      each(root, "[data-om-reveal]", function (el) {
        if (REDUCE) { el.classList.add("om-in"); return; }
        onView(el, function () { el.classList.add("om-in"); }, 0.14);
      });
    },
    count: function (root) {
      each(root, "[data-om-count]", function (el) {
    ...
    ticker: function (root) {
      each(root, "[data-om-ticker]", function (el) {
        var val = String(el.getAttribute("data-om-ticker"));
    ...
    typing: function (root) {
      each(root, "[data-om-typing]", function (el) {
        var full = el.getAttribute("data-om-typing"); var host = el;
    ...
    rotate: function (root) {
      each(root, "[data-om-rotate]", function (el) {
        var words = el.getAttribute("data-om-rotate").split(",");
    ...
    scramble: function (root) {
      ...
      each(root, "[data-om-scramble]", function (el) {
        var target = el.getAttribute("data-om-scramble");
    ...
    magnetic: function (root) {
      if (REDUCE) return;
      each(root, "[data-om-magnetic]", function (el) {
        el.style.transition = "transform .18s " + EASE;
        var s = parseFloat(el.getAttribute("data-om-magnetic")) || 0.3;
    ...
    ripple: function (root) {
      each(root, ".om-ripple", function (el) {
        el.addEventListener("pointerdown", function (e) {
          var r = el.getBoundingClientRect(); var s = document.createElement("span");
          s.className = "om-ink"; s.style.left = (e.clientX - r.left) + "px"; s.style.top = (e.clientY - r.top) + "px";
          el.appendChild(s); setTimeout(function () { s.remove(); }, 700);
        });
      });
    },
    meteors: function (root) {
      each(root, "[data-om-meteors]", function (el) {
        if (REDUCE) return; var n = parseInt(el.getAttribute("data-om-meteors"), 10) || 8;
        el.classList.add("om-meteors");
        for (var i = 0; i < n; i++) { var m = document.createElement("i"); m.className = "om-meteor";
    ...
    particles: function (root) {
      if (REDUCE) return;
      each(root, "[data-om-particles]", function (host) {
        ...
        var col = getComputedStyle(host).getPropertyValue("--accent").trim() || "#FF5722";
